...
expName = 'word priming experiement'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ADMIN\\Documents\\PsychoPy experiments\\word priming experiment\\word priming experiement_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "test_" ---
    ...
    for thisTrial_2 in trials_2:
        ...
        # --- Prepare to start Routine "test_" ---
        # create an object to store info about Routine test_
        test_ = data.Routine(
            name='test_',
            components=[text_2, textbox, text_3, mouse],
        )
        test_.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        text_2.setText(test_)
        textbox.reset()
        ...
        # store start times for test_
        test_.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        test_.tStart = globalClock.getTime(format='float')
        test_.status = STARTED
        thisExp.addData('test_.started', test_.tStart)
        test_.maxDuration = None
        # keep track of which components have finished
        test_Components = test_.components
        for thisComponent in test_.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "test_" ---
        thisExp.currentRoutine = test_
        test_.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # has a Component requested the Routine to end?
            if not continueRoutine:
                test_.forceEnded = routineForceEnded = True
            # has the Routine been forcibly ended?
            if test_.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in test_.components:
        ...
        # --- Ending Routine "test_" ---
        for thisComponent in test_.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for test_
        test_.tStop = globalClock.getTime(format='float')
        test_.tStopRefresh = tThisFlipGlobal
        thisExp.addData('test_.stopped', test_.tStop)
        ...
        if test_.maxDurationReached:
            routineTimer.addTime(-test_.maxDuration)
        elif test_.forceEnded:
            routineTimer.reset()
        else:
